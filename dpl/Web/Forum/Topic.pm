...
# TODO
# Перед всякими там инкрементами, нужно делать select for update
...
# если journal_id не указан, публикуется по обычной схеме
...
#                         $topic_id);
  ...
  # TODO Защита от одинаковых анонсов
  my ($success,$errors);
...
sub nextFilename {
  ...
  $filename=~s/\s/_/g;
  my $f = $filename;
  # TODO действие устанавливать галочкой replace
...
sub attachMusicFile {
  ...
  if ($ext eq 'mp3') {
    ...
    $h{media_genre}=tran($tags->{GENRE}); # нет такого тега, но на всякий случай
  } else {
...
#   
  ...
  # Темы может не быть в журнале, значит ота в черновиках
  if ($res) {
    map {$topic->{$_}=$res->{$_}}
...
sub GetCommentsTree {
  # file_id - файл, который обсуждаем
...
sub _updateAllViewInfo {
  ...
  # ненадо, так как лочится сам топик
  # кажется надо иначе все время баги
  my $list = db()->
...
sub ShowTopic {
  ...
  # Обновляем информацию о последнем просмотре
...
sub ShowFile {
  ...
  my $topic = $self->Get();
  # TODO они должны быть уже такие
  ...
  # непонятно вообще, что это такое
  ...
  if ($uid) {
    ...
    $a{can_upload} = $a{can_edit}
      || $self->Get('upload_access')>=4
        || ($self->Get('upload_access')>=2 && $u->{level}>=1) # все реалы
          || ($self->Get('upload_access')>=3 && $u); # вообще все;
